        ...
        for ups in state.ups_names:
            ...
            with ui.card().props('flat bordered').classes(card_classes):
                with ui.row().classes("items-center gap-4 w-full no-wrap"):
                    ...
                    ).props('unelevated').classes(button_classes).style(
                        f"background-color: {COLOR_THEME['primary']} !important; color: white !important;" if selected else "background-color: #6b7280 !important; color: white !important;"
                    )
