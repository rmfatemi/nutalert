    ...
    with ui.column().classes("w-full gap-y-4"):
        ...
        with ui.row().classes("w-full gap-4").style("align-items: flex-start;"):
            with ui.column().classes("flex-1").style("min-width: 0;"):
                ...
                ).props(f"line-numbers theme={COLOR_THEME['codemirror_theme']}").classes("w-full border").style("height: 60vh; width: 100%;")
            
            with ui.column().classes("flex-1").style("min-width: 0;"):
                ui.label("Live Logs").classes("text-md font-medium mb-2")
                ui_elements["log_view"] = ui.log(max_lines=500).classes(
                    f"w-full bg-[{COLOR_THEME['log_bg']}] font-mono text-sm p-2 rounded border"
                ).style("height: 60vh; width: 100%; overflow-y: auto; flex-shrink: 0;")

        with ui.row().classes("w-full justify-start items-center gap-x-4 mt-4"):
            def save_and_apply():
                try:
